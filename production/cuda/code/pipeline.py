"""
Synthesis Pipeline: End-to-end Python→IR pair generation for CUDA backend.

Pipeline: generate kernel → write to file → import → compile → extract IR → save pair

CUDA MODE: Generates CUDA kernels with PTX/SASS IR instead of CPU C++ IR.
Requires CUDA-capable GPU to run.
"""
import os
import sys
import json
...
'''
    ...
    temp_dir = Path(tempfile.gettempdir()) / "warp_synthesis_cuda"
    temp_dir.mkdir(exist_ok=True)
    
    module_name = f"synth_cuda_{kernel_name}_{source_hash}"
...
def extract_ir_from_kernel(kernel, device: str = "cuda") -> dict[str, str]:
    """Extract IR (generated CUDA code / PTX) from a compiled kernel."""
    ...
    # For CUDA, this generates CUDA C++ code (which gets compiled to PTX)
    cuda_code = builder.codegen(device)
    ...
    if match:
        ...
        for i, char in enumerate(cuda_code[start:], start):
            if char == '{':
                brace_count += 1
                in_function = True
            elif char == '}':
        ...
        forward_code = cuda_code[start:end]
    else:
        forward_code = None
...
def synthesize_pair(spec: KernelSpec, device: str = "cuda") -> dict[str, Any] | None:
    """
    Synthesize a Python→CUDA_IR pair from a kernel specification.
    ...
    try:
        ...
        return {
            ...
            "cuda_ir_forward": ir["forward_code"],
            "metadata": {
...
def synthesize_batch(
    ...
    """
    Synthesize a batch of Python→CUDA_IR pairs.
    
    Args:
        n: Number of pairs to generate
        categories: Optional list of categories to sample from
        seed: Random seed for reproducibility
        device: Target device for IR generation (should be "cuda" or "cuda:0")
...
def run_pipeline(
    n: int = 100,
    output_dir: str = "/workspace/production/cuda/data",
    categories: list[str] | None = None,
    seed: int = 42,
    device: str = "cuda"
):
    """
    Run the full synthesis pipeline for CUDA.
    ...
    Args:
        ...
        seed: Random seed
        device: CUDA device identifier (e.g., "cuda", "cuda:0")
    """
    print("=" * 60)
    print("Warp Kernel Synthesis Pipeline - CUDA MODE")
    print("=" * 60)
    ...
    if not wp.is_cuda_available():
        print("⚠️  WARNING: CUDA not available! Generation will fail.")
        print("   This script requires NVIDIA GPU with CUDA support.")
        print("   Run on a CUDA-capable machine to generate CUDA IR.")
        return []
    ...
    if pairs:
        save_pairs(pairs, output_dir, prefix="cuda_pair")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate Python→CUDA_IR dataset")
    parser.add_argument("-n", "--count", type=int, default=100, help="Number of pairs to generate")
    parser.add_argument("-o", "--output", default="/workspace/production/cuda/data", 
                        help="Output directory")
    parser.add_argument("-s", "--seed", type=int, default=42, help="Random seed")
    parser.add_argument("-c", "--categories", nargs="+", choices=list(GENERATORS.keys()), 
                        help="Categories to generate")
    parser.add_argument("-d", "--device", default="cuda", 
                        help="CUDA device (e.g., 'cuda' or 'cuda:0')")
    
    args = parser.parse_args()
    
    run_pipeline(args.count, args.output, args.categories, args.seed, args.device)

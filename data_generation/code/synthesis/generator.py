...
def generate_nested_loop_kernel(seed: int | None = None) -> KernelSpec:
    """Generate a kernel with nested loops (merged from 9177)."""
    if seed is not None:
        random.seed(seed)
...
'''
    ...
    return KernelSpec(
        ...
        description=f"Nested loop ({outer}x{inner} iterations)",
        metadata={"outer_iters": outer, "inner_iters": inner, "seed": seed}
    )


def generate_multi_conditional_kernel(seed: int | None = None) -> KernelSpec:
    """Generate a kernel with multiple conditions/elif branches (merged from 9177)."""
    if seed is not None:
        random.seed(seed)
...
'''
    ...
    return KernelSpec(
        ...
        description=f"Multi-conditional (thresholds: {t1}, {t2})",
        metadata={"threshold_1": t1, "threshold_2": t2, "seed": seed}
    )


def generate_combined_kernel(seed: int | None = None) -> KernelSpec:
    """Generate a kernel combining loop, conditional, and math (merged from 9177/ff72)."""
    if seed is not None:
        random.seed(seed)
    ...
    func1 = random.choice(["wp.sin", "wp.cos", "wp.exp"])
    func2 = random.choice(["wp.abs", "wp.sqrt"])
...
'''
    ...
    return KernelSpec(
        ...
        source=source,
        arg_types={"a": "wp.array(dtype=float)", "b": "wp.array(dtype=float)", "out": "wp.array(dtype=float)"},
        description=f"Combined: loop({iterations}) + conditional + math",
        metadata={"iterations": iterations, "threshold": threshold, "seed": seed}
    )


def generate_scalar_param_kernel(seed: int | None = None) -> KernelSpec:
    """Generate a kernel with scalar parameters (merged from 9177)."""
    if seed is not None:
        random.seed(seed)
...
'''
    ...
    return KernelSpec(
        ...
        description=f"Scalar parameters with {op} operation",
        metadata={"operation": op, "seed": seed}
    )

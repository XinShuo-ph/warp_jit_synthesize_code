...
def extract_ir_from_kernel(kernel, device: str = "cpu") -> dict[str, str]:
    ...
    import warp.context as ctx  # Changed from _src.context to context if available, or try/except
    
    try:
         # Try public API first if available, else fallback to internal
        if not hasattr(ctx, 'ModuleHasher'):
...
def run_pipeline(
    n: int = 100,
    output_dir: str = "/workspace/data_generation/data/cpu",
    categories: list[str] | None = None,
    seed: int = 42,
    device: str = "cpu"
):
    ...
    try:
        ...
        print(f"Warning: wp.init() failed: {e}. Proceeding anyway.")
    ...
    if pairs:
        save_pairs(pairs, output_dir, prefix=f"synth_{device}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--device", default="cpu", help="Target device (cpu/cuda)")
    
    args = parser.parse_args()
    
    if args.output is None:
        args.output = f"/workspace/data_generation/data/{args.device}"

...
def compile_kernel_batch(
    ...
    # print(f"DEBUG: compiling batch {batch_id} with {len(specs)} specs")
    ...
    # Generate C++ code for the whole module ONCE
    # We can pick any kernel to get the module object
    if not specs:
        return []
    ...
    # print(f"DEBUG: processing kernel {kernel_spec.name}")
        try:
            ...
            # print(f"DEBUG: regex failed for {forward_func_name}")
                # print(f"DEBUG: cpp_code: {cpp_code}") # Too large
                continue
...
def generate_batch(
    ...
    while total_generated < n:
        ...
        for i in range(chunk_n):
            ...
            # Instantiate generator for this iteration to ensure clean state/seed if needed, 
            # though lightweight enough.

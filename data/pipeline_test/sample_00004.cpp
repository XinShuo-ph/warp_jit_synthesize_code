    ...
    wp::float32 var_x)
{
    ...
    //---------
    // forward
    // def helper_4(x: float) -> float:                                                       <L 4>
    // return x * x + 1.0                                                                     <L 5>
    ...
    wp::float32 & adj_ret)
{
    ...
    // adj: def helper_4(x: float) -> float:                                                  <L 4>
    return;
}

struct wp_args_function_0004_448e5ac4 {
    wp::array_t<wp::float32> data;
    wp::array_t<wp::float32> output;
};


void function_0004_448e5ac4_cpu_kernel_forward(
    wp::launch_bounds_t dim,
    size_t task_index,
    wp_args_function_0004_448e5ac4 *_wp_args)
{
    //---------
    // argument vars
    ...
    //---------
    // forward
    // def function_0004(data: wp.array(dtype=float),                                         <L 8>
    // i = wp.tid()                                                                           <L 10>
    var_0 = builtin_tid1d();
    // val = data[i]                                                                          <L 11>
    ...
    var_3 = wp::load(var_1);
    var_2 = wp::copy(var_3);
    // output[i] = helper_4(val)                                                              <L 12>
    var_4 = helper_4_0(var_2);
    wp::array_store(var_output, var_0, var_4);
}



void function_0004_448e5ac4_cpu_kernel_backward(
    wp::launch_bounds_t dim,
    size_t task_index,
    wp_args_function_0004_448e5ac4 *_wp_args,
    wp_args_function_0004_448e5ac4 *_wp_adj_args)
{
    //---------
    // argument vars
    wp::array_t<wp::float32> var_data = _wp_args->data;
    ...
    adj_helper_4_0(var_2, adj_2, adj_4);
    // adj: output[i] = helper_4(val)                                                         <L 12>
    wp::adj_copy(var_3, adj_1, adj_2);
    wp::adj_address(var_data, var_0, adj_data, adj_0, adj_1);
    // adj: val = data[i]                                                                     <L 11>
    // adj: i = wp.tid()                                                                      <L 10>
    // adj: def function_0004(data: wp.array(dtype=float),                                    <L 8>
    return;
}
...
WP_API void function_0004_448e5ac4_cpu_forward(
    wp::launch_bounds_t dim,
    wp_args_function_0004_448e5ac4 *_wp_args)
{
    ...
    {
        function_0004_448e5ac4_cpu_kernel_forward(dim, task_index, _wp_args);
    }
}
...
WP_API void function_0004_448e5ac4_cpu_backward(
    wp::launch_bounds_t dim,
    wp_args_function_0004_448e5ac4 *_wp_args,
    wp_args_function_0004_448e5ac4 *_wp_adj_args)
{
    ...
    for (size_t task_index = 0; task_index < dim.size; ++task_index)
    {
        function_0004_448e5ac4_cpu_kernel_backward(dim, task_index, _wp_args, _wp_adj_args);
    }
}

...
struct wp_args_conditional_0012_eb901afc {
...
void conditional_0012_eb901afc_cpu_kernel_forward(
    wp::launch_bounds_t dim,
    size_t task_index,
    wp_args_conditional_0012_eb901afc *_wp_args)
{
    //---------
    // argument vars
    ...
    //---------
    // forward
    // def conditional_0012(data: wp.array(dtype=float),                                      <L 4>
    // i = wp.tid()                                                                           <L 7>
    var_0 = builtin_tid1d();
    // val = data[i]                                                                          <L 8>
...
void conditional_0012_eb901afc_cpu_kernel_backward(
    wp::launch_bounds_t dim,
    size_t task_index,
    wp_args_conditional_0012_eb901afc *_wp_args,
    wp_args_conditional_0012_eb901afc *_wp_adj_args)
{
    //---------
    // argument vars
    wp::array_t<wp::float32> var_data = _wp_args->data;
    ...
    // adj: val = data[i]                                                                     <L 8>
    // adj: i = wp.tid()                                                                      <L 7>
    // adj: def conditional_0012(data: wp.array(dtype=float),                                 <L 4>
    return;
}
...
WP_API void conditional_0012_eb901afc_cpu_forward(
    wp::launch_bounds_t dim,
    wp_args_conditional_0012_eb901afc *_wp_args)
{
    ...
    {
        conditional_0012_eb901afc_cpu_kernel_forward(dim, task_index, _wp_args);
    }
}
...
WP_API void conditional_0012_eb901afc_cpu_backward(
    wp::launch_bounds_t dim,
    wp_args_conditional_0012_eb901afc *_wp_args,
    wp_args_conditional_0012_eb901afc *_wp_adj_args)
{
    ...
    for (size_t task_index = 0; task_index < dim.size; ++task_index)
    {
        conditional_0012_eb901afc_cpu_kernel_backward(dim, task_index, _wp_args, _wp_adj_args);
    }
}

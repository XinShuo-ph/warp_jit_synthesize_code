...
struct wp_args_arithmetic_0087_da9065ef {
...
void arithmetic_0087_da9065ef_cpu_kernel_forward(
    wp::launch_bounds_t dim,
    size_t task_index,
    wp_args_arithmetic_0087_da9065ef *_wp_args)
{
    //---------
    // argument vars
    ...
    //---------
    // forward
    // def arithmetic_0087(a: wp.array(dtype=float), b: wp.array(dtype=float), c: wp.array(dtype=float)):       <L 4>
    // i = wp.tid()                                                                           <L 5>
    var_0 = builtin_tid1d();
    // c[i] = ((a[i] + b[i]) - b[i])                                                          <L 6>
...
void arithmetic_0087_da9065ef_cpu_kernel_backward(
    wp::launch_bounds_t dim,
    size_t task_index,
    wp_args_arithmetic_0087_da9065ef *_wp_args,
    wp_args_arithmetic_0087_da9065ef *_wp_adj_args)
{
    //---------
    // argument vars
    wp::array_t<wp::float32> var_a = _wp_args->a;
    ...
    // adj: i = wp.tid()                                                                      <L 5>
    // adj: def arithmetic_0087(a: wp.array(dtype=float), b: wp.array(dtype=float), c: wp.array(dtype=float)):  <L 4>
    return;
}
...
WP_API void arithmetic_0087_da9065ef_cpu_forward(
    wp::launch_bounds_t dim,
    wp_args_arithmetic_0087_da9065ef *_wp_args)
{
    ...
    {
        arithmetic_0087_da9065ef_cpu_kernel_forward(dim, task_index, _wp_args);
    }
}
...
WP_API void arithmetic_0087_da9065ef_cpu_backward(
    wp::launch_bounds_t dim,
    wp_args_arithmetic_0087_da9065ef *_wp_args,
    wp_args_arithmetic_0087_da9065ef *_wp_adj_args)
{
    ...
    for (size_t task_index = 0; task_index < dim.size; ++task_index)
    {
        arithmetic_0087_da9065ef_cpu_kernel_backward(dim, task_index, _wp_args, _wp_adj_args);
    }
}

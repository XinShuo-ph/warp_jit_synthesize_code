...
struct wp_args_math_0111_455887e5 {
...
void math_0111_455887e5_cpu_kernel_forward(
    wp::launch_bounds_t dim,
    size_t task_index,
    wp_args_math_0111_455887e5 *_wp_args)
{
    //---------
    // argument vars
    ...
    //---------
    // forward
    // def math_0111(data: wp.array(dtype=float),                                             <L 4>
    // i = wp.tid()                                                                           <L 7>
    var_0 = builtin_tid1d();
    ...
    // output[i] = wp.log(wp.abs(val) + 1.0)                                                  <L 9>
    var_4 = wp::abs(var_2);
...
void math_0111_455887e5_cpu_kernel_backward(
    wp::launch_bounds_t dim,
    size_t task_index,
    wp_args_math_0111_455887e5 *_wp_args,
    wp_args_math_0111_455887e5 *_wp_adj_args)
{
    //---------
    // argument vars
    wp::array_t<wp::float32> var_data = _wp_args->data;
    ...
    // adj: output[i] = wp.log(wp.abs(val) + 1.0)                                             <L 9>
    ...
    // adj: i = wp.tid()                                                                      <L 7>
    // adj: def math_0111(data: wp.array(dtype=float),                                        <L 4>
    return;
}
...
WP_API void math_0111_455887e5_cpu_forward(
    wp::launch_bounds_t dim,
    wp_args_math_0111_455887e5 *_wp_args)
{
    ...
    {
        math_0111_455887e5_cpu_kernel_forward(dim, task_index, _wp_args);
    }
}
...
WP_API void math_0111_455887e5_cpu_backward(
    wp::launch_bounds_t dim,
    wp_args_math_0111_455887e5 *_wp_args,
    wp_args_math_0111_455887e5 *_wp_adj_args)
{
    ...
    for (size_t task_index = 0; task_index < dim.size; ++task_index)
    {
        math_0111_455887e5_cpu_kernel_backward(dim, task_index, _wp_args, _wp_adj_args);
    }
}

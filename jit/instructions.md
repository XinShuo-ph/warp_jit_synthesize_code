...
## Objective
Use Google's `jax` package to extract JIT intermediate representations (JAXPR and XLA HLO) and synthesize Python→IR paired data for LLM training.
...
- `notes/jax_basics.md`: How jit works, where IR lives (max 50 lines)

### M2: IR Extraction Mechanism
**Goal**: Programmatically extract IR from JAX functions
**Deliverables**:
- `code/extraction/ir_extractor.py`: Function that takes a jitted function → returns JAXPR and/or XLA HLO
- 5+ test cases showing Python function → IR pairs
- `notes/ir_format.md`: IR structure documentation (max 30 lines)

### M3: Advanced JAX Features
**Goal**: Understand JAX transformations (vmap, grad, pmap)
**Deliverables**:
- `code/examples/transformations.py`: Examples using vmap, grad, scan
...
- Key APIs to study:
  ...
  - `jax.make_jaxpr` (JAXPR extraction)
  - `jax.xla_computation` (XLA HLO extraction) - deprecated, use lower()
  - `jitted_fn.lower().compile().as_text()` (XLA HLO text)
  - `jax.vmap`, `jax.grad`, `jax.scan` (transformations)

...
def main() -> None:
    ...
    a_np = np.linspace(0.0, 1.0, n, dtype=np.float32)
    b_np = np.linspace(1.0, 2.0, n, dtype=np.float32)
    ...
    print(f"warp_smoke: n={n} device={device} max_err={max_err:.3e} checksum={checksum:.6f}")

    if not np.allclose(out_np, expected, rtol=0.0, atol=1e-6):
        raise SystemExit("warp_smoke failed: output mismatch")

...
# Allow running both as a script and as an importable module.
# When executed as a script, the directory containing this file is on sys.path,
# so `import generator` works. When imported as a package, prefer relative imports.
_EXTRACTION_DIR = Path(__file__).parent.parent / "extraction"
if str(_EXTRACTION_DIR) not in sys.path:
    sys.path.insert(0, str(_EXTRACTION_DIR))

import warp as wp
try:
    from .generator import generate_kernel, generate_kernels, KernelSpec, GENERATORS  # type: ignore
except Exception:
    from generator import generate_kernel, generate_kernels, KernelSpec, GENERATORS

wp.set_module_options({"enable_backward": False})

def _warp_device_aliases() -> set[str]:
    aliases: set[str] = set()
...
def extract_ir_from_kernel(kernel, device: str = "cpu") -> dict[str, str]:
    """Extract generated code from a compiled kernel (cpu/cuda)."""
    try:
        from ir_extractor import extract_ir  # script mode (path injected above)
    except Exception:
        from ..extraction.ir_extractor import extract_ir  # type: ignore

    result = extract_ir(kernel, device=device, include_backward=False)
    return {
        "full_code": result["cpp_code"],
        "forward_code": result["forward_code"],
        "code_ext": result["metadata"]["code_ext"],
    }
...
def synthesize_pair(spec: KernelSpec, device: str = "cpu") -> dict[str, Any] | None:
    ...
    try:
        ...
        pair: dict[str, Any] = {
            ...
            "code_full": ir["full_code"],
            "metadata": {
                "kernel_name": spec.name,
                "category": spec.category,
                "description": spec.description,
                "device": device,
                "code_ext": ir["code_ext"],
                **spec.metadata
            }
        }
        # Back-compat field names.
        if device == "cpu":
            pair["cpp_forward"] = ir["forward_code"]
        else:
            pair["cu_forward"] = ir["forward_code"]
        return pair
...
def run_pipeline(
    ...
    # Fail fast if a CUDA run was requested but no CUDA device exists.
    if device == "cuda" and "cuda" not in _warp_device_aliases():
        raise RuntimeError("Requested device=cuda but warp reports no CUDA device. Run on a GPU machine.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device", choices=["cpu", "cuda"], default="cpu", help="Target backend for codegen")

...
def extract_ir_from_kernel(kernel, device: str = "cpu", include_backward: bool = False) -> dict[str, str]:
    ...
    options["enable_backward"] = include_backward
...
def synthesize_pair(spec: KernelSpec, device: str = "cpu", include_backward: bool = False) -> dict[str, Any] | None:
    """
    Synthesize a Python→IR pair from a kernel specification.
    
    Args:
        spec: Kernel specification
        device: "cpu" for C++ output, "cuda" for CUDA output
        include_backward: If True, include backward/adjoint kernels
    ...
    try:
        ...
        result = {
            ...
            "metadata": {
                ...
                "device": device,
                "ir_type": "cuda" if device == "cuda" else "cpu",
                **spec.metadata
            }
        }
...
def synthesize_batch(
    ...
    Args:
        ...
        device: Target device for IR generation ("cpu" or "cuda")
        include_backward: If True, include backward/adjoint kernels
    ...
    for i, spec in enumerate(specs):
        ...
        pair = synthesize_pair(spec, device, include_backward)
        if pair is not None:
            pairs.append(pair)
...
def run_pipeline(
    ...
    if pairs:
        # Use different prefix for CUDA vs CPU
        prefix = f"synth_{device}" if device != "cpu" else "synth"
        save_pairs(pairs, output_dir, prefix=prefix)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Warp Kernel Synthesis Pipeline")
    ...
    parser.add_argument("-d", "--device", default="cpu", choices=["cpu", "cuda"],
                        help="Target device for IR generation (cpu=C++, cuda=CUDA)")
    ...
    run_pipeline(args.n, args.output, args.categories, args.seed, args.device, args.backward)

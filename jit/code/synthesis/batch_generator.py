...
def generate_batch(
    ...
    if device == "cuda" and not wp.is_cuda_available():
        print("WARNING: CUDA device requested but not available.")
        print("         Generating CUDA IR code anyway (for testing on GPU device).")
        print()
    
    print(f"Generating {n} pairs ({device}) in chunks of {chunk_size}...")
    ...
    while total_generated < n:
        ...
        for batch_start in range(0, len(specs), KERNELS_PER_MODULE):
            ...
            pairs = compile_kernel_batch(batch_specs, batch_id, temp_dir, device)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Large-scale Warp kernel synthesis")

...
try:
    from .generator import generate_kernel, GENERATORS, KernelSpec  # type: ignore
except Exception:
    ...
    for kernel_spec in specs:
        try:
            ...
            pair: dict[str, Any] = {
                ...
                "code_forward": forward_code,
                "code_full": result["cpp_code"],
                "metadata": {
            ...
            if device == "cpu":
                pair["cpp_forward"] = forward_code
            else:
                pair["cu_forward"] = forward_code
            pairs.append(pair)
...
def generate_batch(
    ...
    if device == "cuda" and "cuda" not in aliases:

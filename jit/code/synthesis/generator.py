...
ARRAY_TYPES = [
    "array",  # JAX arrays (float by default)
    "array_int",  # Integer arrays
]
...
def generate_loop_kernel(seed: int = None) -> str:
    ...
    return f'''@jax.jit
def {name}(a):
    """Loop operation: accumulate {n} times."""
    def body_fn(carry, _):
...
def generate_vector_kernel(seed: int = None) -> str:
    ...
    ops = [
        ("jnp.dot(a, b)", "dot product"),
        ("jnp.linalg.norm(a)", "vector norm"),
        ("jnp.cross(a, b)", "cross product"),
    ]
    op_code, op_desc = random.choice(ops[:2])  # dot or norm
    
    if "dot" in op_code or "cross" in op_code:
        return f'''@jax.jit
def {name}(a, b):
    """Vector operation: {op_desc}."""
    return {op_code}
'''
    else:
        return f'''@jax.jit
...
def generate_kernel_batch(count: int, base_seed: int = 42) -> List[str]:
    ...
    for i in range(count):
        func = generate_random_kernel(base_seed + i)

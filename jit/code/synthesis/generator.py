...
# Global generator type registry for batch generation
GENERATORS = {
    ...
    "vector": "gen_vector",
    "atomic": "gen_atomic",
    "nested": "gen_nested_loop",
    "multi_cond": "gen_multi_conditional",
    "combined": "gen_combined",
    "scalar_param": "gen_with_scalar_param",
}


@dataclass
class KernelSpecExtended:
    """Extended kernel spec with source code for batch generation."""
    name: str
    source: str
    category: str
    description: str
...
def generate_kernel(category: str, seed: int = None) -> KernelSpecExtended:
    """Generate a kernel of specified category with source code."""
    gen = KernelGenerator(seed=seed)
    spec = gen.generate(category)
    source = gen.to_python_source(spec)
    
    return KernelSpecExtended(
        name=spec.name,
        source=source,
        category=category,
        description=f"Auto-generated {category} kernel",
        metadata={"num_params": len(spec.params), "num_lines": len(spec.body_lines)}
    )

...
def compile_kernel_batch(
    ...
    for i, kernel_spec in enumerate(specs):
        try:
            ...
            # CUDA kernel name usually: <mangled>_cuda_kernel_forward
            
            import re
            # __global__ void <mangled>_cuda_kernel_forward( ... )
            pattern = rf'void\s+\w*{kernel_spec.name}\w*_cuda_kernel_forward\s*\('
            ...
            if match:
                ...
                if end_func != -1:
                    ...
                    pairs.append({
                        ...
                        "cuda_ir_forward": forward_code,
                        "cuda_ir_backward": "", 
                        "cuda_ir_full": cpp_code,

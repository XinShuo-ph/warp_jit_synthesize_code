...
def extract_kernel_ir(kernel, cpp_full: str) -> tuple:
    ...
    forward_pattern = rf'void\s+{re.escape(kernel_name)}_[a-f0-9]+_cuda_kernel_forward\('
    backward_pattern = rf'void\s+{re.escape(kernel_name)}_[a-f0-9]+_cuda_kernel_backward\('
...
class SynthesisPipeline:
    ...
    def generate_pair(self, kernel_type: Optional[str] = None) -> Optional[SynthesisPair]:
        ...
        try:
            ...
            # Get cache path and read generated CUDA
            ...
            if not cpp_file.exists():
                raise FileNotFoundError(f"CUDA IR not found: {cpp_file}")

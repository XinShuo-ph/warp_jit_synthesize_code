...
def extract_kernel_ir(kernel, cpp_full: str, device: str = "cpu") -> tuple:
    ...
    forward_pattern = rf'void\s+{re.escape(kernel_name)}_[a-f0-9]+_{device}_kernel_forward\('
    backward_pattern = rf'void\s+{re.escape(kernel_name)}_[a-f0-9]+_{device}_kernel_backward\('
...
class SynthesisPipeline:
    ...
    def generate_pair(self, kernel_type: Optional[str] = None) -> Optional[SynthesisPair]:
        ...
        try:
            ...
            # Determine source file based on device
            if self.device == "cuda":
                src_file = cache_dir / module_id / f"{module_id}.cu"
            else:
                src_file = cache_dir / module_id / f"{module_id}.cpp"
            
            if not src_file.exists():
                # Fallback check
                if self.device == "cuda" and (cache_dir / module_id / f"{module_id}.cpp").exists():
                     src_file = cache_dir / module_id / f"{module_id}.cpp"
                else:
                    raise FileNotFoundError(f"IR source file not found: {src_file}")
            
            src_full = src_file.read_text()
            forward_ir, backward_ir = extract_kernel_ir(kernel, src_full, device=self.device)
            
            if not forward_ir:
                # Debug: try ignoring device name in pattern if strictly following conventions failed
                raise ValueError(f"Failed to extract forward IR for {self.device}")
            ...
            self.stats["failed"] += 1
            # Only print detailed error if it's not the expected "CUDA driver not found" which might occur in this environment
            msg = str(e)
            if "CUDA" not in msg and "driver" not in msg:
                 print(f"Failed to generate pair for {spec.name}: {e}")
            else:
                 # If we are failing due to missing driver, we can't really proceed with success, 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device", type=str, default="cpu",
                        help="Device to target (cpu or cuda)")

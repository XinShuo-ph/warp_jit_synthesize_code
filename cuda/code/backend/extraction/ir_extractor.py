...
def extract_ir(kernel: wp.Kernel, device: str = "cpu") -> IRPair:
    ...
    ext = "cu" if device == "cuda" else "cpp"
    cpp_file = cache_dir / f"{module_id}.{ext}"
    
    if not cpp_file.exists():
        raise FileNotFoundError(f"Generated {ext.upper()} not found at {cpp_file}")
...
def extract_kernel_functions(cpp_ir: str, kernel_name: str, device: str = "cpu") -> dict:
    ...
    # Determine kernel suffix based on device
    suffix = "cuda_kernel" if device == "cuda" else "cpu_kernel"
    
    # Find the mangled kernel name (includes hash suffix)
    # Pattern: kernel_name_HASH_device_kernel_forward
    forward_pattern = rf'void\s+{re.escape(kernel_name)}_[a-f0-9]+_{suffix}_forward\([^)]*\)\s*\{{[^}}]*\}}'
    backward_pattern = rf'void\s+{re.escape(kernel_name)}_[a-f0-9]+_{suffix}_backward\([^)]*\)\s*\{{[^}}]*\}}'
    ...
    forward_match = re.search(
        rf'(void\s+{re.escape(kernel_name)}_[a-f0-9]+_{suffix}_forward\([^)]*\)\s*\{{)',
        cpp_ir
    )
    if forward_match:
    ...
    backward_match = re.search(
        rf'(void\s+{re.escape(kernel_name)}_[a-f0-9]+_{suffix}_backward\([^)]*\)\s*\{{)',
        cpp_ir
    )
    if backward_match:

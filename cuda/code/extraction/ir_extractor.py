...
def extract_ir(kernel: wp.Kernel, device: str = "cpu") -> IRPair:
    ...
    if device == "cuda":
        src_file = cache_dir / f"{module_id}.cu"
    else:
        src_file = cache_dir / f"{module_id}.cpp"
    
    if not src_file.exists():
        # Fallback for some configurations where it might be .cpp even for cuda? 
        # Or if we want to be safe
        fallback = cache_dir / f"{module_id}.cpp"
        if device == "cuda" and fallback.exists():
            src_file = fallback
        else:
            raise FileNotFoundError(f"Generated IR file not found at {src_file}")
...
def extract_kernel_functions(cpp_ir: str, kernel_name: str, device: str = "cpu") -> dict:
    ...
    # Pattern: kernel_name_HASH_{device}_kernel_forward
    # Note: re.escape escapes everything, so we need to be careful with building regex
    ...
    forward_match = re.search(
        rf'(void\s+{re.escape(kernel_name)}_[a-f0-9]+_{device}_kernel_forward\([^)]*\)\s*\{{)',
        cpp_ir
    )
    if forward_match:
    ...
    backward_match = re.search(
        rf'(void\s+{re.escape(kernel_name)}_[a-f0-9]+_{device}_kernel_backward\([^)]*\)\s*\{{)',
        cpp_ir
    )
    if backward_match:

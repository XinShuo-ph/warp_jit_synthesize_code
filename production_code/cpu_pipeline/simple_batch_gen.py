"""
Simple Production Script for CPU Python→IR Pairs
Generates large batches of training data efficiently.
"""
import os
import sys
import json
...
def extract_ir_from_kernel(kernel):
...
def generate_single_pair(gen, kernel_type=None):
    ...
    try:
        # Import module
        module_name = f"temp_kernel_{random.randint(0, 999999)}"
        ...
        ir_code = extract_ir_from_kernel(kernel)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate CPU Python→IR training pairs")

"""Fast batch generator for large-scale CUDA Python→IR pair generation."""
import os
import sys
import json
import hashlib
...
def extract_function(pattern_start: str, code: str) -> str:
    """Extract a function from C++/CUDA code."""
    import re
...
def generate_batch_cuda(
    ...
    """Generate n Python→CUDA IR pairs efficiently.
    
    Generates CUDA code using Warp's codegen for cuda device.
    No GPU required - just generates the CUDA C++ code.
    """
    import warp as wp
    ...
    print(f"Generating {n} CUDA pairs to {output_dir}")
        ...
        try:
            ...
            for i, (kspec, src, ktype) in enumerate(zip(specs, sources, kernel_types)):
                ...
                try:
                    ...
                    # Get module and generate CUDA code directly using codegen
                    ...
                    # Generate CUDA code instead of CPU code
                    ...
                    # Extract forward and backward CUDA functions
                    kernel_key = kernel.key
                    
                    # CUDA kernel functions use __global__ and different naming
                    forward_pattern = rf'__global__ void\s+{re.escape(kernel_key)}_[a-f0-9]+_cuda_kernel_forward\('
                    backward_pattern = rf'__global__ void\s+{re.escape(kernel_key)}_[a-f0-9]+_cuda_kernel_backward\('
                    ...
                    filename = f"{pair_id}_{kspec.name}_cuda.json"
    ...
    stats["pairs_per_second"] = stats["successful"] / stats["elapsed_seconds"] if stats["elapsed_seconds"] > 0 else 0
...
def print_stats(stats: Dict[str, Any]):
    ...
    print("\n" + "=" * 60)
    print("CUDA Generation Complete")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fast batch generator for CUDA Python→IR pairs")
    parser.add_argument("-n", type=int, default=1000, help="Number of pairs to generate")
    parser.add_argument("-o", "--output", type=str, default="data/cuda", help="Output directory")
    parser.add_argument("-s", "--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--kernels-per-module", type=int, default=10, help="Kernels per compilation batch")
    
    args = parser.parse_args()
    
    stats = generate_batch_cuda(
        n=args.n,

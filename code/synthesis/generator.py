...
def generate_multi_conditional_kernel(seed: int | None = None) -> KernelSpec:
    ...
    t2 = round(t1 + random.uniform(1, 5), 2)
...
def generate_combined_kernel(seed: int | None = None) -> KernelSpec:
    """Generate a kernel combining loops, conditionals, and math."""
    if seed is not None:
        random.seed(seed)
    ...
    func = random.choice(["wp.sin", "wp.cos", "wp.sqrt", "wp.abs"])
...
def generate_scalar_param_kernel(seed: int | None = None) -> KernelSpec:
    ...
    op = random.choice(["+", "-", "*"])
    scale = round(random.uniform(0.1, 5.0), 2)
    offset = round(random.uniform(-2, 2), 2)
